...
int main()
{
	...
	// populates the queue
	string vectorNumbers[] = {"Alex", "Dan", "Steve", "Joel", "Mary", "Karen", "John", "Fred", "Michelle", "Eve", "Tom", "Brenda", "Peta", "Steve", "Daniel", "Cameron", "Josephus", "Ysobel", "Tim", "Ben", "Anna", "Allan", "Ben"};
	int numValues = sizeof(vectorNumbers)/sizeof(vectorNumbers[0]);
    cout << numValues << endl;

	cout << "This is a demo for DanSort for strings, which is a made up thing." << endl << endl; 
	...
	{
		counter = 0;
		while ((!myStacks[counter].isEmpty()) && (myStacks[counter].peek() < myQueue.front())) {counter++; numComparisons++;}
	...
	for (int j=0; j<numValues; j++)
	{
		...
		for (int i=0; i<numStacks; i++)
		{
			...
			{
				numComparisons++;
				if (myStacks[i].peek() < min) {min = myStacks[i].peek(); minStack = i;}
			}
		}
